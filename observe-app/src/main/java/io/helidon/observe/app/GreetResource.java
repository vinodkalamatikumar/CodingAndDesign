...
public class GreetResource {
     ...
     */
    @GET
    @Counted(name = "greetCountGetDefaultMessage")  
    @Produces(MediaType.APPLICATION_JSON)
    public Message getDefaultMessage() {

        HelidonMdc.set("traceid", configuredTracer.activeSpan().context().toTraceId());
        HelidonMdc.set("spanid", configuredTracer.activeSpan().context().toSpanId() );
        LOGGER.info("Log from getDefaultMessage");
    ...
    @Path("/{name}")
    @GET
    @Counted(name = "greetCountGetMessage")  
    @Produces(MediaType.APPLICATION_JSON)
    public Message getMessage(@PathParam("name") String name) {

        HelidonMdc.set("traceid", configuredTracer.activeSpan().context().toTraceId());
        HelidonMdc.set("spanid", configuredTracer.activeSpan().context().toSpanId() );
        LOGGER.info("Log from getMessage where name is " + name);
